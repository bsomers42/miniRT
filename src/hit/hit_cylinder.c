...
/*   Updated: 2022/11/18 13:53:10 by bsomers       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
// float	quadratic_form_cyl(t_cyl *cyl, t_ray rot_ray, float *t_orig, float t_max)
// {
// 	float	a;
// 	float	b;
// 	float	c;
...
// 	t[0] = (-b+sqrt(d))/(2*a);
// 	t[1] = (-b-sqrt(d))/(2*a);
// 	if ((t[0] < (float)0.01 && t[1] < (float)0.01) || (t[0] > t_max && t[1] > t_max))
// 		return (0);
// 	if ((t[0] < t[1] && t[0] > (float)0.01) || t[1] < (float)0.01)
// 		t[2] = t[0];
// 	else
// 		t[2] = t[1];
// 	t_orig = t;
...
int	hit_tube(t_cyl *cyl, t_ray ray, /*float t_min,*/ float t_max, t_besthit *hit_rec)
{
	...
	// t[2] = quadratic_form_cyl(cyl, rot_ray, t, t_max);
	a = rot_ray.dir.x * rot_ray.dir.x + rot_ray.dir.z * rot_ray.dir.z;
	b = 2*(rot_ray.origin.x * rot_ray.dir.x + rot_ray.origin.z * rot_ray.dir.z);
	c = rot_ray.origin.x * rot_ray.origin.x + rot_ray.origin.z * rot_ray.origin.z - pow(cyl->radius, 2); //dit laatste is bij ons niet 1 omdat we zowel hoogte + diam hebben
	d = b*b - 4*a*c;
	...
	t0 = (-b+sqrt(d))/(2*a);
	t1 = (-b-sqrt(d))/(2*a);
	if ((t0 < 0.01 && t1 < 0.01) || (t0 > t_max && t1 > t_max))
		return (0);
	if ((t0 < t1 && t0 > 0.01) || t1 < 0.01)
		t = t0;
	else
		t = t1;
...
int	hit_any_cylinder(t_parse map_info, t_ray ray, t_besthit *hit_rec, float t_min, float t_max)
{
	...
	while (tmp)
	{
		if (hit_both_caps((t_cyl *)tmp->content, ray, t_min, t_max, &tmp_rec))
		{
			hit_anything = i;
			t_max = tmp_rec.t;
			*hit_rec = tmp_rec;
		}
		if (hit_tube((t_cyl *)tmp->content, ray, /*t_min,*/ t_max, &tmp_rec))
		{
			hit_anything = i;
			t_max = tmp_rec.t;
			*hit_rec = tmp_rec;
