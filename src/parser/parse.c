...
/*   Updated: 2022/10/12 13:03:58 by bsomers       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	make_list(char **line)
{
	
}

void	assign_to_struct(char **map_split_n)
{
	int	i;
	int a;
	int c;
	...
	while (map_split_n[i])
	{
		...
		free(tmp);
		if (a > 1 || l > 1 || c > 1)
			error_exit("too many light(ning) or camera defined", 1);
		i++;
	}
}
